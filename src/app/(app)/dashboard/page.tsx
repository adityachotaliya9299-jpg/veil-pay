...
import { WalletMultiButton } from "@solana/wallet-adapter-react-ui";
import { useUmbraClient } from "@/hooks/useUmbraClient";
import { Lock, Zap, Eye, ArrowRight } from "lucide-react";
import Link from "next/link";

export default function DashboardPage() {
  const { connected, publicKey } = useWallet();
  const { status, client } = useUmbraClient();
  ...
  try {
    ...
    const queryFn =
      sdk.getEncryptedBalanceQuerierFunction ||
      sdk.getEncryptedBalanceFunction ||
      sdk.getPrivateBalanceQuerierFunction;

    if (!queryFn) { setEncBalance("N/A"); return; }

    const query = queryFn({ client });
    const USDC = process.env.NEXT_PUBLIC_NETWORK === "mainnet"
      ? "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"
      : "4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU";

    const result = await (query as any)(publicKey!.toBase58() as any, USDC as any);
    const raw = result?.balance ?? result?.amount ?? result?.encryptedBalance ?? result;
    const num = typeof raw === "bigint" ? Number(raw) : Number(raw ?? 0);
    setEncBalance(num > 0 ? (num / 1_000_000).toFixed(2) : "0.00");
  } catch {
    setEncBalance("0.00");
  }
};
    ...
    return (
      ...
      >
        ...
        <div style={{ textAlign: "center" }}>
          ...
          >
            Use Phantom or Solflare on Solana
          </p>
        </div>
      ...
      >
        {[
            ...
            value:
              status === "ready"
                ...
                : status === "initializing"
                  ? "Connecting..."
                  : "Idle",
            dot: status === "ready" ? "var(--green)" : "var(--cyan)",
          },
...

          {
            ...
            value: encBalance ? `${encBalance} USDC` : "—",
            dot: "var(--green)",
          },
        ...
        {[
          {
            ...
            desc: "Send private payments to your team",
          },

          {
            href: "/compliance",
            icon: <Eye size={16} color="var(--cyan)" />,
            label: "Compliance View",
            desc: "Decrypt transactions with viewing key",
          },
        ].map((a) => (
          <Link key={a.href} href={a.href} style={{ textDecoration: "none" }}>
            <div
