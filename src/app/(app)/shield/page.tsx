...
import { Shield, Lock, Loader2, CheckCircle, AlertCircle, ArrowDown, ExternalLink } from "lucide-react";
  ...
  return (
    <div style={{ maxWidth:"600px" }}>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:"12px",marginBottom:"32px",padding:"16px 20px",borderRadius:"10px",background:"var(--bg-card)",border:"1px solid var(--border)" }}>
        <div style={{ textAlign:"center" }}>
          <p style={{ fontFamily:"var(--font-mono)",fontSize:"10px",color:"var(--text-muted)",marginBottom:"4px" }}>PUBLIC WALLET</p>
        ...
        <div style={{ flex:1,textAlign:"center",padding:"8px 12px",borderRadius:"8px",background:"var(--cyan-dim)",border:"1px solid var(--border-accent)" }}>
          <p style={{ fontFamily:"var(--font-mono)",fontSize:"10px",color:"var(--cyan)",marginBottom:"2px" }}>UMBRA MIXER</p>
        ...
        <div style={{ textAlign:"center" }}>
          <p style={{ fontFamily:"var(--font-mono)",fontSize:"10px",color:"var(--text-muted)",marginBottom:"4px" }}>ENCRYPTED BALANCE</p>
      ...
      <div style={{ padding:"28px",borderRadius:"12px",background:"var(--bg-card)",border:"1px solid var(--border)",marginBottom:"24px" }}>
        ...
        <div style={{ position:"relative",marginBottom:"20px" }}>
          ...
          />
          <span style={{ position:"absolute",right:"16px",top:"50%",transform:"translateY(-50%)",fontFamily:"var(--font-mono)",fontSize:"13px",color:"var(--text-muted)" }}>{token.split(" ")[0]}</span>
        </div>
        ...
        <button onClick={shield} disabled={status === "shielding" || !client || !amount} style={{
          width:"100%",padding:"14px",borderRadius:"10px",cursor: !client || !amount ? "not-allowed" : "pointer",border:"none",
          background: !client || !amount ? "var(--bg-card-hover)" : "var(--cyan)",
          color: !client || !amount ? "var(--text-muted)" : "#050810",
          fontFamily:"var(--font-display)",fontWeight:700,fontSize:"15px",
          display:"flex",alignItems:"center",justifyContent:"center",gap:"10px",
          boxShadow: client && amount ? "0 0 30px rgba(0,212,255,0.25)" : "none",transition:"all 0.2s"
        }}>
          {status === "shielding"
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginBottom:"32px" }}>
        {[
          { title:"After shielding", items:["Balance hidden on-chain","Only you can query it","Use for private payroll","Fully auditable with viewing key"] },
          { title:"ZK proof ensures", items:["No link between deposit & balance","Amount stays private","Cryptographic guarantee","Arcium MPC co-signs"] },
        ].map(card => (
      ...
      {history.length > 0 && (
        <div>
          ...
          {history.map((h, i) => (
            <div key={i} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",borderRadius:"8px",background:"var(--bg-card)",border:"1px solid var(--border)",marginBottom:"8px" }}>
              <div style={{ display:"flex",alignItems:"center",gap:"10px" }}>
                <CheckCircle size={14} color="var(--green)" />
                <div>
                  <p style={{ fontFamily:"var(--font-display)",fontWeight:600,fontSize:"13px",color:"var(--text)" }}>{h.amount} {h.token.split(" ")[0]} shielded</p>
